...
bucket_size = 600
bucket = bucket_size
p_size = 100 #value zero indicates random packets size
rate = 24000
exceed = 0
conform = 0
...
j = 0
step = round(p_size*8/pr,2) #step for constant packets size
while currentTime <= endTime:
    if counter == t_on + 0.02:
        currentTime = round(currentTime + t_off - 0.02,2)
        counter = 0
    else:
        #step = round(packets[j]*8/pr,2) #step for random packets size
        ...
        currentTime = round(currentTime + step,2)
